      ...
      <section className="container mx-auto px-4 pb-20 space-y-6">
        ...
        {appointments.map((a) => (
          <div key={a.id} className="border rounded-xl p-6 bg-card/40 shadow-sm">
            ...
            <p className="text-sm mt-1 text-muted-foreground">{a.date} — {a.time}</p>

            <p className="mt-2"><span className="font-medium">Client:</span> {a.name}</p>
            ...
            <p className="text-sm mt-3 font-semibold">
              Status:{" "}
              <span className={a.status === "Pending" ? "text-yellow-500" : a.status === "Approved" ? "text-green-500" : "text-red-500"}>
                {a.status}
              </span>
            </p>

            <div className="flex gap-3 mt-4">
              <Button size="sm" variant="glass" onClick={() => approveBooking(a.id)}>Approve</Button>
              <Button size="sm" className="bg-yellow-600 hover:bg-yellow-700 text-white" onClick={() => cancelBooking(a.id)}>Cancel</Button>
              <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => deleteBooking(a.id)}>Delete</Button>
            </div>
          </div>
        ))}
      </section>
